...
class Renderer {
  async renderDocumentation(automations, pipe, mailTemplates, flags) {
    ...
    twing.addFilter(new TwingFilter('replaceHandlebars', function (text) {
      ...
      fieldsReplaced = text.replaceAll(/\{\{[ ]*assignees.name[ ]*\}\}/g, translate('<span  class="handlebar-replacement">' + translate('Name of Assignees') + '</span>'))
      fieldsReplaced = text.replaceAll(/\{\{[ ]*assignees.email[ ]*\}\}/g, translate('<span  class="handlebar-replacement">' + translate('E-Mail of Assignees') + '</span>'))
      return Promise.resolve(fieldsReplaced)
    }))
    twing.addFilter(new TwingFilter('replacePercentagebars', function (text) {
      let fieldsReplaced = text.replaceAll(/%\{(?:[0-9.]*)*\.(\d*)[ ]*\}/ig, '<a href="#field-$1" class="handlebar-replacement">' + translate('Field') + ' #$1</a>')
      return Promise.resolve(fieldsReplaced)
    }))
